...
Format stable validé en production:
- file_id simple
- custom: True
...
Ce script:
1. Récupère les posts/drafts cassés
2. Reprend leur image Wix existante  
3. Refait le PATCH avec le format qui marche
4. Vérifie que media est bien enregistré
"""
...
async def migrate_broken_drafts(
    api_key: str,
    site_id: str,
    ...
    for item in broken_items:
        ...
        height = item.get("height", 630)

        ok = await patch_cover_v2(
            api_key=api_key,
            site_id=site_id,
            draft_id=draft_id,
            file_id=file_id,
        ...
        if ok:
            results["fixed"].append(draft_id)
        else:
            results["failed"].append(draft_id)
...
async def main(dry_run: bool = True):
    logger.info("=" * 60)
    logger.info("MIGRATION DES COVERS CASSÉES - V2 STABLE")
    logger.info(f"Mode: {'DRY-RUN (simulation)' if dry_run else 'FIX (application réelle)'}")
    logger.info("Format: file_id simple + custom: True")
    logger.info("=" * 60)
    ...
    for post in posts:
        ...
        if image_id and len(image_id) > 60:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fix broken blog covers V2 STABLE")
